...
QUINTILE_COLORS = ["#86b6ef", "#5598e7", "#2a78d6", "#1c5cab", "#0d366b"]
...
def main():
    ...
    cut_points = sorted(df.groupby("risk_quintile")["percent_chance"].min().to_numpy()[1:] * 100)
    ...
    for c in bin_centers:
        q = next(i for i in range(5) if boundaries[i] <= c < boundaries[i + 1])
        bar_colors.append(QUINTILE_COLORS[q])
    ...
    legend_handles = [plt.Rectangle((0, 0), 1, 1, color=c) for c in QUINTILE_COLORS]
    ax.legend(legend_handles, QUINTILE_LABELS, title="Risk quintile", loc="upper right",
