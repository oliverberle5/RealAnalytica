"""Calibration layer: raw CatBoost score -> (percent chance, risk quintile).
...
Quintile cutpoints are equal-population (20% each) on the PRODUCTION
calibrator's output. Per the "High bucket hides a spread" finding, each
output includes the calibrated percentage AND a risk multiplier
(percentage / base rate) alongside the quintile label, not the label alone.
...
to predictions for the quarter after. Backtest: a calibrator fit on
...
QUINTILE_LABELS = ["Low", "Medium-Low", "Medium", "Medium-High", "High"]
...
def compute_recent_bias_correction(per_fold, recent_fold_idx: int) -> float:
    ...
    retrain cycle (see HANDOFF.md "Next up" #3, the not-yet-built quarterly
    pipeline) using whichever fold has just become fully known.
    """
...
def fit_production_calibrator(per_fold):
    ...
    calibrated = platt.predict_proba(all_scores.reshape(-1, 1))[:, 1]
    base_rate = all_labels.mean()
    cut_points = np.quantile(calibrated, [0.2, 0.4, 0.6, 0.8])
...
def score_to_output(raw_score: float, platt: LogisticRegression, cut_points, base_rate: float,
    ...
    return {
        ...
        "risk_quintile": QUINTILE_LABELS[bucket_idx],
...
def main():
    ...
    print(f"quintile cut points (calibrated %): {[f'{c:.3%}' for c in cut_points]}")
    ...
    for raw in [0.05, 0.2, 0.35, 0.5, 0.65, 0.8, 0.95]:
        ...
        print(
            ...
            f"{out['risk_multiplier']:.1f}x base rate, bucket={out['risk_quintile']}"
        )
