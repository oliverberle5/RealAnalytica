...
re-derives the office-pair catalogue first, runs the Sales model off the same
snapshot afterwards, checks both outputs (including that they still agree on
key column, roster and snapshot) before letting either be considered shipped,
and records what the calibration did this quarter. Running this file directly
still works and produces the same scores -- it just skips all of that, which is
exactly how the 2026-07-30 output went three weeks stale on a retired
identifier with nobody noticing.

Retrains the model and calibrator fresh each run rather than loading a saved
artifact, so it always reflects whatever data.py / crossval.py / calibrate.py
currently do -- there is no model-versioning/drift-between-runs risk to manage
yet, at the cost of a few minutes of retraining time per run.
"""
...
# Written alongside the CSV on every run. Two reasons it is a file rather than
# just a return value: the quarterly wrapper runs this script as a SUBPROCESS
# (so the office-pair catalogue it regenerates first is guaranteed to be the
# one this run imports, and so the Sales model's identically-named `data` /
# `score_agents` modules cannot collide with these in one interpreter), and a
# hand-run also leaves behind the three numbers that define the quarter's
# calibration instead of burying them in console scrollback.
META_PATH = "score_meta.json"
