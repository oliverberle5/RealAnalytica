...
def main():
    ...
    # The top-5 Extra-High-tier MA agents as of the 2026-07-01 snapshot.
    #
    # 2026-08-30: repointed to real MLSPIN agents. This list had been DEAD
    # SINCE THE PORT, not merely stale -- it was copied verbatim from the RI
    # project and every id in it was a RIAR agent (agent_profile_ids resolving
    # to 38002 Curtis Lopes, 34684, 45328, 44863, 49350). MA's load_clean
    # filters mls_code == "mlspin", so those agents can never appear here, and
    # the lookup loop below skips ids it cannot find -- so the demo silently
    # printed nothing on every run since 2026-07-28. Crosswalking the UUIDs to
    # MLS ids would NOT have fixed it: the ids were valid, just from the other
    # board. Note for the next identifier change: re-point pinned constants
    # against the panel the module actually loads, not just the id format.
    #
    # This is a fixed historical sample, so it drifts out of the live top-5 as
    # scores move; that is intended -- the loop skips any id no longer present.
    highlight_ids = [
        "BB301780",  # Cynthia Tamburro, Advisors Living - Weston
        "B8001048",  # Meredith Fahey, Advisors Living - Tewksbury
        "CN255628",  # Marlene Reyes Rolon, 1 Worcester Homes
        "CN206611",  # Jasmine McCarthy, Advisors Living - Canton
        "CN207701",  # Martha Morese, Suburban Lifestyle Real Estate
    ]
    print("\n" + "=" * 100)
