...
from calibrate import (
    ...
    score_to_output, RISK_TIER_LABELS,
)
...
def _header(fig, title, subtitle=None, page_num=None):
    ...
    fig.text(0.08, 0.025, "Agent Likelihood-to-Leave Model  |  MLSPIN", fontsize=9, color=INK_MUTED, ha="left")
...
def main():
    ...
    n_agents = df["mls_agent_id"].nunique()
    n_rows = len(df)
    ...
    n_tiers = len(RISK_TIER_LABELS)
    tier_predicted, tier_actual, tier_n = [], [], []
    for i in range(n_tiers):
        mask = bucket_idx == i
        tier_predicted.append(calibrated_pct[mask].mean())
        tier_actual.append(all_labels[mask].mean())
        tier_n.append(mask.sum())
    ...
    with PdfPages(out_path) as pdf:
        ...
        fig.text(0.08, 0.79, "Massachusetts (MLSPIN) real estate agents  |  July 2026", fontsize=12, color="#cfe0f5", ha="left")

        fig.text(0.08, 0.60, "74.3%", fontsize=64, fontweight="bold", color=BLUE_DARK, ha="left")
        ...
        body = (
            ...
            "multiplier vs. the base rate, and a 4-tier risk label (Low → Extra High)."
        )
        ...
        fig.text(0.10, 0.115,
                  "Wins 5 of 5 folds, best mean AUC and PR-AUC, and lift-at-top-decile of "
        ...
        # -------------------------------------------------- Page 4: calibration
        fig = _new_page()
        _header(fig, "Calibration: Do the Percentages Mean What They Say?", "Predicted vs. actual departure rate, by risk tier (out-of-fold, bias-corrected)", page_num=4)

        ax = fig.add_axes([0.10, 0.52, 0.80, 0.32])
        x = np.arange(n_tiers)
        w = 0.35
        b1 = ax.bar(x - w / 2, np.array(tier_predicted) * 100, w, color=BLUE_LIGHT, label="Predicted", zorder=3)
        b2 = ax.bar(x + w / 2, np.array(tier_actual) * 100, w, color=BLUE_DARK, label="Actual", zorder=3)
        for bars in (b1, b2):
            for b in bars:
                h = b.get_height()
                ax.text(b.get_x() + b.get_width() / 2, h + 0.15, f"{h:.1f}%", ha="center", fontsize=8.5, color=INK_SECONDARY)
        ax.set_xticks(x)
        ax.set_xticklabels(RISK_TIER_LABELS)
        ax.set_ylabel("3-month departure rate")
        ax.spines[["top", "right"]].set_visible(False)
        ax.yaxis.grid(True, color=GRID, linewidth=0.8, zorder=0)
        ax.set_axisbelow(True)
        ax.legend(loc="upper left", frameon=False, fontsize=9)
        ax.set_title("Risk tier: predicted vs. actual", fontsize=11, color=INK_SECONDARY, loc="left")

        fig.text(0.08, 0.44, "Risk output per agent", fontsize=12, fontweight="bold", color=NAVY, ha="left")
        table_y = 0.32
        col_x = [0.08, 0.28, 0.52, 0.70]
        headers = ["Risk tier", "Cutpoint (calibrated %)", "Actual rate", "Risk multiplier"]
        ...
        cut_labels = (
            ["< " + f"{cut_points[0]:.2%}"]
            + [f"{cut_points[i-1]:.2%}–{cut_points[i]:.2%}" for i in range(1, len(cut_points))]
            + [f"≥ {cut_points[-1]:.2%}"]
        )
        for i, (ql, cl, act) in enumerate(zip(RISK_TIER_LABELS, cut_labels, tier_actual)):
        ...
        rows = [
            ("Prediction horizon", "3 months (not 12) — unlocks 95.4% of rows vs. 81.2%, matches “who's a flight risk right now.”"),
            ...
            ("Label cleanup", "2,810 rows recoded from “left” to “stayed” across the MA rebrand/M&A/mass-mover office\ntransitions (≥5 agents, same origin/destination, same quarter) — corporate events, not\nvoluntary departures."),
            ...
            ("Class imbalance", f"Handled via class-weighting (not synthetic oversampling) — only {base_rate:.3%} of rows are positive."),
        ...
        fig.text(0.08, 0.12,
                  f"{n_rows:,} agent-quarter records, {n_agents:,} unique MLSPIN (Massachusetts) agents, "
