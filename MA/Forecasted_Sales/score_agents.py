...
and unit count for every currently active MLSPIN (MA) agent, at a fixed menu of
confidence levels (decision, HANDOFF.md): 50/65/70/75/80/95%.

MA PORT (2026-07-28): identical to the RI pipeline in
`RI/Forecasted_Sales/score_agents.py` -- not one line of
modeling logic differs. The state switch lives entirely in data.py's MLS_CODE.
Note that the brand one-hots here are built dynamically by pd.get_dummies over
whatever office_brand values are present, so MA's extra `bhgre` brand is picked
up automatically (unlike the Leave model, whose brand list is hardcoded and had
