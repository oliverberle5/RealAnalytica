...
# ever carried four. `volume_trend_vs_market_12m` (-1) is not in
# PRODUCTION_FEATURES -- the AutoResearch 94->54 pruning dropped it, keeping
# `units_trend_vs_market_12m` instead -- and since the constraint vector is
# built with MONOTONE.get(c, 0), the missing key silently resolved to
# "unconstrained" rather than erroring. Resolved in favour of the model: the
# key is removed, so declared and deployed now agree at four. Dropping it from
# TREE_FEATURES as well (via _fit_xgboost, the one path where it did bind)
# costs +0.000881 mean / -0.000248 worst-fold AUC on the reference XGBoost --
# inside the noise band, below the +0.001 keep bar. `units_trend_vs_market_12m`
# is deliberately left unconstrained rather than inheriting the sign: it
# survived pruning on its own merits and never carried a validated prior.
MONOTONE = {
